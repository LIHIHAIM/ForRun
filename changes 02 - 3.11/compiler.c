...
boolean compile(char *fileName){
     ...
     while (FOREVER){
          ...
          case directive:
               ...
               case string:
                    if (labelExist){
                         if (isIlegalName(optLabel)) /* if label was all ready defined */
                              compSuc = FALSE;
                         else if (command == data){
                              if (!addToSymTab(optLabel, "data"))
                                   return FALSE;
                         }
                         else if (command == string){
                              if (!addToSymTab(optLabel, "string"))
                                   return FALSE;
                         }
                    }
          ...
          case order:
               ...
               /* Lihis enum */
               if (labelExist){
                    if (isIlegalName(optLabel)){
                         compSuc = FALSE;
                         continue;
                    }
                    if(!addToSymTab(optLabel, "code"))
                         compSuc = FALSE;
                    /*push(".code", IC, optLabel)
                    IC++;*/
               }
               if(!(pushed = pushInst(line, lInd)))
                    compSuc = FALSE;
               else if(pushed == ERROR)
                    return ERROR;
