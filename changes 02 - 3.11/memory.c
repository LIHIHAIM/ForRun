...
boolean pushExtern(char *line, int *lInd){
    ...
    *label = readLabel(line, lInd);
    if(!isValidLabel(label) || isIlegalName(*label) || !addToSymTab(*label, 0, "external"))
        return FALSE;
    return TRUE;
}
...
word *scanInstParams(char *line, int *lInd){
    int i = 0;
    char *curr;
    word *params;
    word *error;
    ...
    while(line[i] != '\0'){
        curr = getOperand(line,lInd);
        if(curr == NULL)
            return NULL;
        if(curr[0] == '\0')
            return params;
        if(!isThereComma(line, lInd) || !isBlank(line, lInd)){
            printf("error in instruction line: operand expected after comma\n");
            return error;
        }
...
char *getOperand(char *line, int *lInd){
    int size = 1;
    word *param = calloc(size, sizeof(word));
    if(!isAlloc(param))
        return NULL;
    while(line[*lInd] != '\0' && !isspace(line[*lInd])){
        param = realloc(param, ++size * sizeof(word));
        if(!isAlloc(param))
            return NULL;
        parma[size-1] = line[*lInd];
        (*lInd)++;
    }
    param[size] = '\0';
    return param;
